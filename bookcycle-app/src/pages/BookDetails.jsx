...
// Assuming ChatButton is a separate component you have
  ...
  const handleRequest = async () => {
    ...
    try {
      const { error } = await supabase.from('requests').insert({
        ...
        from_user_name: user.email, // Using email as name for simplicity
        ...
        <div>
          ...
          <div className="mt-6 space-y-3">
            <button
              ...
              className="w-full bg-green-600 text-white py-3 rounded-lg font-semibold hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
            >
              {isOwner ? "This is your book" : book.is_borrowed ? "Currently Borrowed" : requestSent ? 'Request Sent!' : 'Request to Borrow'}
            </button>
            {/*!isOwner && user && (
              <ChatButton
                currentUser={user}
